...
void ActivityUpdater::startupdate(QString activityname)
{
    ...
    if(!activitydir.exists()) activitydir.mkdir(runtimedir + "/data/activities/" + activityname);
    ...
    for(QString filename:filelist)
    {
        ...
        if(fileinfo.suffix() == "0")
        {
            ...
            if(!localfilelist.contains(filename))
            {
                ...
                QObject::connect(bigfiledownloader,&BigFileDownloader::downloadfinished,this,[activityname,this](){this->dealdownloadfinished(activityname);});
        ...
        {
            Downloader* downloader = new Downloader(memorybase::generaterequesturl(reponame,filename),activitypath + "/" + filename);
            QObject::connect(downloader,&Downloader::downloadfinished,this,[activityname,this](){this->dealdownloadfinished(activityname);});
            taskcount[activityname]++;
            downloadmanager->adddownloader(downloader);
        }
    }
    if(--repocount[activityname]) return;
    if(taskcount[activityname]) return;
